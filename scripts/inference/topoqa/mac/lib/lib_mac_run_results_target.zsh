...
usage() {
  cat <<USAGE
Usage: $0 -d DATASET_NAME -t TARGET -g GROUND_TRUTH_FILE -r TOPO_RESULTS_DIR -f TOPO_RESULT_FILE -l LOG_FILE
  or   $0 --dataset-name NAME --target NAME --ground-truth-file FILE --topo-results-dir DIR --topo-result-file NAME --log-file FILE
All six parameters are mandatory (no defaults).
USAGE
}

# Allow env overrides but do NOT set defaults
...
# Ensure log directory exists and redirect logs
log_dir="${LOG_FILE%/*}"
[ "$log_dir" != "$LOG_FILE" ] && mkdir -p "$log_dir" || true
