...
DATASET_NAME="${DATASET_NAME:-example}"
DATASET_DIR="${DATASET_DIR:-../../../../datasets/examples/BM55-AF2/decoy}"
GROUND_TRUTH_FILE="${GROUND_TRUTH_FILE:-../../../../datasets/examples/BM55-AF2/label_info.csv}"
...
INF_SCRIPT="${INF_SCRIPT:-${script_dir}/lib/lib_mac_run_inference.zsh}"
...
while (( i <= total )); do
  ...
  if DATASET_NAME="$DATASET_NAME" DATASET_DIR="$DATASET_DIR" TARGET_DIR="$target" \
    ...
    LOG_FILE="${script_dir}/results_${DATASET_NAME}_${target}_${ts}.log" \
    "$RESULTS_SCRIPT"
