...
def test_edge_plus_bal_agg_lap_hybrid_shapes(tmp_path):
    ...
    module = EdgePlusBalAggLapHybridModule(scale_histogram=False)  # keep histogram block unscaled for assertions
...
def test_edge_plus_bal_agg_lap_hybrid_heavy_minmax(tmp_path):
    residues, id_to_index = _build_residues()
    structure = _DummyStructure()
    node_df = pd.DataFrame({"ID": [res.descriptor for res in residues], "feat": [1.0, 2.0]})
    topo_vectors = [[1.0, 0.5, 0.25], [0.2, 0.3, 0.4]]
    ...
    module = EdgePlusBalAggLapHybridModule(scale_histogram=False, variant="heavy", include_minmax=True)
    result = module.build_edges(
