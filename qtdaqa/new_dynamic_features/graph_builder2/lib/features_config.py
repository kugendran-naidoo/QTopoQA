...
def load_feature_config(config_path: Optional[Path]) -> FeatureSelection:
    ...
    for section, entry in data.items():
        if section in DEFAULT_FEATURES or section in META_SECTIONS:
            continue
        if isinstance(entry, Mapping) and "module" in entry:
            ...
            alias = extras[section].get("alias")
            if alias in (None, ""):
                alias = alias_map.get(entry["module"])
                if alias:
                    extras[section]["alias"] = alias
            continue
        # Unsupported structure handled in _validate_schema
...
def _validate_schema(data: Mapping[str, Any], config_path: Path) -> None:
    ...
    for section, entry in data.items():
        if section in REQUIRED_SECTIONS or section in META_SECTIONS:
            continue
        if isinstance(entry, Mapping) and "module" in entry:
            if section not in DEFAULT_FEATURES:
                LOG.info(
                    "Feature config %s defines custom stage '%s'. Ensure the pipeline can consume this stage.",
                    config_path,
                    section,
                )
            continue
