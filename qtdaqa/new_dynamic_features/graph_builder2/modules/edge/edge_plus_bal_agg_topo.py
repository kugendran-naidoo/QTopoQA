...
class EdgePlusBalAggTopoModule(EdgeFeatureModule):
    ...
    def build_edges(
        ...
        *,
        ...
        if variant != "lean":
            raise ValueError(f"Unsupported variant '{variant}' for edge_plus_bal_agg_topo (lean only in v1).")
        ...
        agg_dim = topo_dim * 4 + (2 if include_norms else 0) + (1 if include_cosine else 0)
        ...
        metadata: Dict[str, Any] = {
            ...
            "edge_feature_variant": "edge_plus_bal_agg_topo/lean",
            "variant": variant,
    ...
    def config_template(cls) -> Dict[str, object]:
        ...
        param_comments.setdefault("variant", "lean only (heavy will add min/max later)")
