...
class PersistenceFullKPartiteModule(TopologyFeatureModule):
    """
    Fully k-partite persistent homology with primary partitions (chains) and optional secondary (DSSP/chemotype).

    Presets:
      - minimal: base PH only (140D)
      - lean (default): base + cross-only per primary pair + per-primary PH
      - heavy: lean + cross-biased + optional polar mini + guardrails
      - heavy_stratified: heavy + stratified blocks on primary×secondary labels (DSSP/chemotype)
    """

    module_id = "topology/persistence_full_k_partite/v1"
    module_kind = "topology"
    default_alias = "Fully k-partite PH (chains + optional secondary stratification) = preset-dependent"
    _metadata = build_metadata(
        module_id=module_id,
        module_kind=module_kind,
        summary="Persistent homology with partition-aware blocks (primary partitions and optional secondary stratification).",
        description=(
            "Extends partial k-partite PH to arbitrary primary partitions (chains by default) and an optional secondary "
            "layer (DSSP or chemotype). Builds per-partition and per-pair PH blocks, with stratified variants in the "
            "heavy_stratified preset. Emits dims/schemas for downstream adaptation."
        ),
    ...
    def validate_params(cls, params: Dict[str, Any]) -> None:
        ...
        for key in ("enable_cross_bias", "enable_polar_block", "polar_hbond_weight"):
            val = params.get(key)
            if val is not None:
                params[key] = require_bool(val, f"topology.params.{key}")
        for key in ("polar_hbond_inter_only",):
            val = params.get(key)
            if val is not None:
                params[key] = require_bool(val, f"topology.params.{key}")
    ...
    def config_template(cls) -> Dict[str, object]:
        ...
        base["alias"] = "Fully k-partite PH (lean) = 560D"
        ...
        param_comments = {
            "preset": "minimal=140D, lean=base+cross-pair+per-partition (minimum 560D for k=2), heavy=~700+ D (cross-bias+options), heavy_stratified=~900+ D (heavy + secondary stratification)",
            "secondary_partition": "none|dssp|chemotype (mutually exclusive; ignored by lean/minimal; heavy_stratified uses one secondary layer)",
            "k_max": "Cap on number of primary partitions; blocks beyond cap are skipped (used by lean/heavy/stratified)",
            "secondary_k_max": "Cap on number of secondary partitions (for stratified); ignored by lean/minimal/heavy",
            "element_filters_stratified": "Reduced filters for stratified blocks (default C/N/O); used only in heavy_stratified",
            "enable_cross_bias": "Used only in heavy/heavy_stratified; ignored by lean/minimal even if true",
            ...
            "intra_penalty_lambda": "Used only when intra_penalty_mode=custom; otherwise derived from neighbor_distance; heavy/heavy_stratified only (ignored by lean/minimal)",
            ...
            "enable_polar_block": "Used only in heavy/heavy_stratified; ignored by lean/minimal",
            "polar_hbond_weight": "Toggle H-bond weighting for polar block (heavy/heavy_stratified only); ignored by lean/minimal",
            "polar_hbond_weight_factor": "Distance multiplier for H-bonded polar atoms (default=0.5); heavy/heavy_stratified only",
            "polar_hbond_energy_cutoff": "DSSP H-bond energy cutoff (kcal/mol) to consider bonded (default=-0.5); heavy/heavy_stratified only",
            "polar_hbond_inter_only": "If true, only weight inter-chain H-bonded atoms in polar block; heavy/heavy_stratified only",
        }
        base["params"] = params
        ...
        minimal_comments.update(
            {
                ...
                "preset": "standard topoqa = 140D = topology/persistence_basic/v1",
            }
        )
        alternates = [
            {
                ...
                "params": {**params, "preset": "minimal", "enable_cross_bias": False, "enable_polar_block": False},
                "param_comments": minimal_comments,
            },
            {
                "module": cls.module_id,
                "alias": "Fully k-partite PH (heavy) = 700+ D",
                "params": {**params, "preset": "heavy", "enable_cross_bias": True, "enable_polar_block": False},
...
def _run_full_single(
    *,
    ...
    expected_cols = base_config.max_rips_dimension * len(base_config.element_filters) * 5 + base_config.max_alpha_dimension * len(base_config.element_filters) * 5  # rough placeholder
    ...
    if preset == "heavy_stratified" and secondary_partition != "none" and heavy_allowed:
