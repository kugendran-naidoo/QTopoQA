...
# Smoke regression for topology/persistence_k_partite_advanced_laplacian_only/v2 (fixed-width schema).
# Runs minimal/lean/heavy/heavy_stratified on a tiny dataset and asserts metadata/schema invariants.
#
# Usage: ./exec_smoke_regression_advanced_lap_only_v2.sh [dataset_dir] [jobs]
...
run_smoke() {
  local preset_name="$1"
  local cfg_path="$2"
  local output_dir="${SCRIPT_DIR}/output/smoke_advanced_lap_only_v2_${preset_name}"
  ...
  echo "== Running lap-only v2 preset ${preset_name} with config ${cfg_path} ==" >&2
...
CFG_BASE="${SCRIPT_DIR}/../feature_configs/advanced_k_partite_lap_only_v2"
run_smoke "minimal" "${CFG_BASE}/feature-config.yaml.advanced_lap_only_v2_minimal.10A"
run_smoke "lean" "${CFG_BASE}/feature-config.yaml.advanced_lap_only_v2_lean.10A"
run_smoke "heavy" "${CFG_BASE}/feature-config.yaml.advanced_lap_only_v2_heavy.10A"
run_smoke "heavy_stratified" "${CFG_BASE}/feature-config.yaml.advanced_lap_only_v2_heavy_stratified.10A"

echo "Advanced lap-only v2 smoke regression passed."
