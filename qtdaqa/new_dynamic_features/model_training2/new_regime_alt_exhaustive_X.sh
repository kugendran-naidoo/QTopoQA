# General
export OUTPUT=topoqa_10A_ARM
export CHECK=topoqa_10A_ARM
...
RUN_FILTER="exhaustive_alt_"


echo "Starting alternative exhaustive model training ..."

# run initial model training
./run_full_pipeline.sh --manifest manifests_exhaustive/run_lean_exhaustive_alt.yaml | \
tee ${OUTPUT}_run_lean_exhaustive_alt_$(date +%Y%m%d_%H%M%S).log

echo "Starting alternative exhaustive post model training polishing ..."

# run post training polish from best
RUN_FILTER="${RUN_FILTER}" \
LAUNCH=1 MANIFEST_PATH=manifests_exhaustive/run_polish_exhaustive_alt.yaml \
./polish_from_best_exhaustive.sh | \
tee ${OUTPUT}_exhaustive_alt_polish_from_best_$(date +%Y%m%d_%H%M%S).log
