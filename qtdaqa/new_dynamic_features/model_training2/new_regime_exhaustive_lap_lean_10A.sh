# General
export OUTPUT=lap_lean_10A_ARM
export CHECK=lap_lean_10A_ARM
...
echo "Starting exhaustive model training ..."

# run initial model training
./run_full_pipeline.sh --manifest manifests_exhaustive/run_lean_exhaustive.yaml | \
tee ${OUTPUT}_run_lean_exhaustive_$(date +%Y%m%d_%H%M%S).log

echo "Starting exhaustive post model training polishing ..."

# run post training polish from best
RUN_FILTER="${RUN_FILTER}" \
LAUNCH=1 MANIFEST_PATH=manifests_exhaustive/run_polish_exhaustive.yaml \
./polish_from_best_exhaustive.sh | \
tee ${OUTPUT}_exhaustive_polish_from_best_$(date +%Y%m%d_%H%M%S).log

echo "Auditing EMA metrics after polish ..."
