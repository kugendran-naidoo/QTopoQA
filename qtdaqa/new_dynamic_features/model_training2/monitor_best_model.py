...
--------------
  ...
  python monitor_best_model.py --run-dir qtdaqa/new_dynamic_features/model_training2/training_runs2/latest
...
def _to_repo_relative(path_value: Optional[str]) -> Optional[str]:
    ...
    for resolver in (candidate.resolve, lambda: candidate):
        ...
        for base in bases:
            ...
            try:
                return str(
                    (Path("training_runs2") / resolved.relative_to(base))
    ...
    for base in bases:
        ...
        try:
            return str(
                (Path("training_runs2") / candidate.relative_to(base))
                if base == getattr(train_cli, "RUN_ROOT", None)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser.add_argument("--run-id", type=str, help="Run identifier under training_runs2/ (e.g., training_run_2025-01-01_12-00-00).")
