...
def _resolve_run_dir_argument(run_id: Optional[str], run_dir: Optional[Path]) -> Path:
    ...
    if run_dir:
        if run_dir.is_absolute():
            ...
            base = RUN_ROOT.parent if run_dir.parts and run_dir.parts[0] == "training_runs2" else REPO_ROOT
    ...
    for resolver in (candidate.resolve, lambda: candidate):
        try:
            ...
            if resolved.is_relative_to(RUN_ROOT):
                return str(Path("training_runs2") / resolved.relative_to(RUN_ROOT))
            return str(resolved.relative_to(REPO_ROOT))
        except Exception:
            continue
    try:
        if candidate.is_relative_to(RUN_ROOT):
            return str(Path("training_runs2") / candidate.relative_to(RUN_ROOT))
    except Exception:
        pass
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  ./run_model_training.sh summarise --run-dir training_runs2/history/training_run_2025-10-28_18-16-16\n"
            "\n"
            "Direct module invocation:\n"
            "  python -m qtdaqa.new_dynamic_features.model_training2.train_cli run --config config.yaml\n"
        ),
    )
    ...
    resume_parser.add_argument("--run-id", type=str, help="Run identifier under training_runs2/ (e.g., training_run_2025-01-01_12-00-00).")
    ...
    summarise_parser.add_argument("--run-id", type=str, help="Run identifier under training_runs2/.")
