...
export TOP_K
# when true, also run each selected top-K run’s averaged_ema.* checkpoint if present
INCLUDE_EMA=true
export INCLUDE_EMA
...
WORK_DIR="topoqa_10A_no_drift_ARM_training2_new_regime"
...
if [[ "${INCLUDE_EMA}" == "true" ]]; then
  for idx in "${!CHECKPOINT_PATHS[@]}"; do
    ckpt_path="${CHECKPOINT_PATHS[$idx]}"
    run_name="${CHECKPOINT_RUNS[$idx]}"
    run_dir="$(dirname "$(dirname "${ckpt_path}")")"
    if [[ -z "${run_name}" || "${run_name}" == "${ckpt_path}" ]]; then
      run_name="$(basename "${run_dir}")"
    fi
    ema_candidates=()
    while IFS= read -r file; do
      ema_candidates+=("$file")
    done < <(ls -1 "${run_dir}/model_checkpoints"/averaged_ema.* 2>/dev/null | sort || true)
    if [[ ${#ema_candidates[@]} -gt 0 ]]; then
      ema_path="${ema_candidates[0]}"
      CHECKPOINT_PATHS+=("${ema_path}")
      CHECKPOINT_IDS+=("ema_${run_name}")
      CHECKPOINT_RUNS+=("${run_name}")
    fi
  done
fi
