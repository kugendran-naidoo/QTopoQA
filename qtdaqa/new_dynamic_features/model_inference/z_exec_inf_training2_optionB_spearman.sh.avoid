#!/usr/bin/env bash
# Run inference for Option B (val_loss top-K, best tuning spearman) checkpoint.
set -euo pipefail
...
CKPT_PATH="$("${PYTHON_BIN}" -m qtdaqa.new_dynamic_features.model_training2.tools.option_b_select \
  --top-k "${TOP_K}" \
  --shortlist-metric "${SHORTLIST_METRIC}" \
  --tuning-metric "${TUNING_METRIC}" \
  --emit-checkpoint-only)"

if [[ -z "${CKPT_PATH}" ]]; then
  echo "Error: option_b_select returned empty checkpoint path." >&2
  exit 1
fi

CKPT_ID="$(basename "${CKPT_PATH}")"
CKPT_ID="${CKPT_ID%.chkpt}"
CKPT_ID="${CKPT_ID#checkpoint.}"
...
echo "Option B checkpoint: ${CKPT_PATH}"
