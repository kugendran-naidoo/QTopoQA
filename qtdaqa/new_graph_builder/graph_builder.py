...
from config import BuilderConfig, config_as_dict, load_config
...
# configuration placeholder updated at runtime
CURRENT_CONFIG: Optional[BuilderConfig] = None
...
def _apply_runtime_config(config: BuilderConfig) -> None:
    global INTERFACE_CUTOFF, INTERFACE_COORD_DECIMALS
    global TOPOLOGY_NEIGHBOR_DISTANCE, TOPOLOGY_FILTRATION_CUTOFF, TOPOLOGY_MIN_PERSISTENCE
    global NODE_FEATURE_DROP_NA, CURRENT_CONFIG

    CURRENT_CONFIG = config
    INTERFACE_CUTOFF = float(config.interface.cutoff)
    INTERFACE_COORD_DECIMALS = int(config.interface.coordinate_decimals)
    TOPOLOGY_NEIGHBOR_DISTANCE = float(config.topology.neighbor_distance)
    TOPOLOGY_FILTRATION_CUTOFF = float(config.topology.filtration_cutoff)
    TOPOLOGY_MIN_PERSISTENCE = float(config.topology.min_persistence)
    NODE_FEATURE_DROP_NA = bool(config.node_drop_na)
...
def _configure_main_logger(run_log_dir: Path) -> logging.Logger:
    ...
    file_handler = logging.FileHandler(run_log_dir / "graph_builder.log", encoding="utf-8")
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser = HelpOnErrorArgumentParser(
        ...
        usage=usage,
        description="Configurable graph builder for QTopoQA (with enhanced multi-scale edges).",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    builder_config = load_config(config_path if config_path.exists() else None)
    _apply_runtime_config(builder_config)

    try:
        log_info: LogDirectoryInfo = prepare_log_directory(log_root, run_prefix="graph_builder")
    except Exception as exc:
        print(f"Error: unable to prepare log directory '{log_root}': {exc}", file=sys.stderr)
        return 2

    run_log_dir = log_info.run_dir
    summary_log_path = run_log_dir / "graph_builder_summary.log"
    logger = _configure_main_logger(run_log_dir)
    logger.info("=== Starting graph_builder run ===")
    logger.info("Config file: %s (exists=%s)", config_path, config_path.exists())
    logger.info(
        "Resolved parameters: %s",
        json.dumps(config_as_dict(builder_config), indent=2, sort_keys=True),
    )

    raw_args = list(argv) if argv is not None else sys.argv[1:]
    logger.info("CLI raw arguments: %s", raw_args)
    logger.info(
        "CLI parameters: dataset_dir=%s, work_dir=%s, graph_dir=%s, log_dir=%s, jobs=%s",
        args.dataset_dir,
        args.work_dir,
        args.graph_dir,
        args.log_dir,
    ...
    topology_config = TopologicalConfig(
        ...
        workers=None,
        log_progress=False,
        dedup_sort=builder_config.topology.dedup_sort,
    )
    ...
    if pt_result.edge_feature_dim is not None:
        logger.info("Edge feature dimension: %d", pt_result.edge_feature_dim)
    if pt_failures:
    ...
    summary_lines.append("=== Graph Builder Summary ===")
    ...
    if pt_result.edge_feature_dim is not None:
        summary_lines.append(f"  Edge feature dim : {pt_result.edge_feature_dim}")
    if pt_failures:
        summary_lines.append("  Failure details:")
        for model, error, log_path in pt_failures:
            summary_lines.append(f"    - {model}: {error} (log: {log_path})")
    ...
    manifest = {
        ...
        "config": config_as_dict(builder_config),
        "counts": {
            "interfaces": generated_interface_files,
            "topology": generated_topology_files,
            "node_features": node_feature_success,
            "graphs_processed": pt_processed,
            "graphs_success": pt_success_count,
            "graphs_failures": len(pt_failures),
        },
        "missing": {
            "interfaces": len(interface_failure_entries),
            "topology": len(topology_failure_entries),
            "node_features": len(node_feature_failures),
        },
        "edge_feature_dim": pt_result.edge_feature_dim,
    }
    manifest_path = run_log_dir / "run_manifest.json"
    manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    logger.info("Manifest written to %s", manifest_path)

    logger.info("=== graph_builder run completed ===")
